...
export class WelcomeCustomComponent implements OnInit, AfterViewInit {
	...
	convertOnLoad() {
		...
		this.indexService.getRate(payload).subscribe(
			(response: any) => {
				...
				this.otherValue = parseFloat(
					(this.baseValue * parseFloat(this.rateData.receivingCurrencyAmount)).toFixed(2)
				);
	...
	filterCurrencies() {
		this.cryptoCurrencies = this.currencies.filter((currency) => currency.isCrypto);
		this.baseCurrencies = this.currencies.filter((currency) => currency.isBaseCurrency);
		this.otherCurrencies = this.currencies.filter((currency) => !currency.isCrypto && !currency.isBaseCurrency);
	...
	convertBase(event?) {
		...
		this.getRate(this.selectedCoin.code, this.selectedBaseCurrency.code, this.selectedOtherCurrency.code);

		this.otherValue = parseFloat((this.baseValue * parseFloat(this.rateData.receivingCurrencyAmount)).toFixed(2));
		this.coinValue = parseFloat((this.baseValue * parseFloat(this.rateData.sendingCurrencyAmount)).toFixed(6));
		// this.addCommasBaseValue();
		this.addCommasOtherValue();
	...
	convertOther(event?) {
		...
		this.baseValue = parseFloat((this.otherValue / parseFloat(this.rateData.receivingCurrencyAmount)).toFixed(2));
		this.coinValue = parseFloat((this.baseValue * parseFloat(this.rateData.sendingCurrencyAmount)).toFixed(6));
		this.addCommasBaseValue();
		this.addCommasCoinValue();
		// this.addCommasOtherValue();
